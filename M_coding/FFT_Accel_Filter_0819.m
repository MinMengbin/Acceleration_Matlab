load('C:\Users\mm395\Documents\MATLAB\REC0819ch2.mat'); % load acceleration data
figure;plot(REC0819ch2(:,1),REC0819ch2(:,2));
title('Acceleration data');
fft_accel = fft(REC0819ch2(:,2)); % Compute the FFT of the acceleration data
L_accel = length(REC0819ch2(:,2)); % Calculate the number of the data
...
%filtered_accel = filter(b, a, REC0819ch2(:,2) );
% 
filtered_accel = filter(Hd, REC0819ch2(:,2) );

figure;plot(REC0819ch2(:,1),filtered_accel);
...
figure; plot(REC0819ch2(:,1),AC_V_acce);
title('Speed');
xlabel('Time (s)');
ylabel('Velocity (m/s)');
...
%T = table(time',AC_V_acce','VariableNames',{'Time','CumulativeDistance'});
figure; plot(REC0819ch2(:,1),AD_V_acce);
title('Displacement');
xlabel('Time (s)');
