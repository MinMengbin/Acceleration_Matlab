...
%filtered_accel = filter(b, a, REC0826ch2(:,2) );
% 
filtered_accel = filter(Hd, REC0826ch2(:,2) );

figure;plot(REC0826ch2(:,1),filtered_accel);
...
AD_100_V_acce = cumtrapz(AC_V_acce)*(1/Fs);
%T = table(time',AC_V_acce','VariableNames',{'Time','CumulativeDistance'});
figure; plot(REC0826ch2(:,1),AD_100_V_acce);
title('Displacement');
xlabel('Time (s)');
