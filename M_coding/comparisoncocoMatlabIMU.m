% this coding is used to compare the data of Matlab real-time with CoCo80

load('C:\Users\mm395\Documents\MATLAB\REC2100.mat'); % load acceleration data
figure;plot(REC2100(:,1),REC2100(:,2));
title('Acceleration data');
fft_accel = fft(REC2100(:,2)); % Compute the FFT of the acceleration data
L_accel = length(REC2100(:,2)); % Calculate the number of the data
...
%filtered_accel = filter(b, a, REC2100(:,2) );
% 
filtered_accel = filter(Hd, REC2100(:,2) );

figure;plot(REC2100(:,1),filtered_accel);
...
figure; plot(REC2100(:,1),AC_V_acce);
title('Speed');
xlabel('Time (s)');
ylabel('Velocity (m/s)');
...
%T = table(time',AC_V_acce','VariableNames',{'Time','CumulativeDistance'});
figure; plot(REC2100(:,1),AD_V_acce);
title('Displacement');
xlabel('Time (s)');
