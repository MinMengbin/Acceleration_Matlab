...
figure;plot(REC2098(:,1),REC2098(:,2));
title('Acceleration data');
fft_accel = fft(REC2098(:,2)); % Compute the FFT of the acceleration data
L_accel = length(REC2098(:,2)); % Calculate the number of the data
...
%filtered_accel = filter(b, a, REC2098(:,2) );
% 
filtered_accel = filter(Hd, REC2098(:,2) );

figure;plot(REC2098(:,1),filtered_accel);
...
figure; plot(REC2098(:,1),AC_V_acce);
title('Speed');
xlabel('Time (s)');
ylabel('Velocity (m/s)');
...
%T = table(time',AC_V_acce','VariableNames',{'Time','CumulativeDistance'});
figure; plot(REC2098(:,1),AD_V_acce);
title('Displacement');
xlabel('Time (s)');
